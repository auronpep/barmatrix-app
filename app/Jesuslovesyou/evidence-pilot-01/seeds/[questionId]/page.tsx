...
import {
  evidenceSeedQuestionParams,
  getEvidenceSeedCandidate,
...
export function generateStaticParams() {
  return evidenceSeedQuestionParams;
}
...
export async function generateMetadata({
  ...
  const seedCandidate = getEvidenceSeedCandidate(questionId);
  if (!seedCandidate) {
    return {};
  }
  ...
  if (!detail) {
    return {
      title: `Q${seedCandidate.question_id} - Evidence Seed Review Row`,
      description: `Evidence-Pilot-01 seed review row for Q${seedCandidate.question_id}.`,
      alternates: {
        canonical: `/Jesuslovesyou/evidence-pilot-01/seeds/${seedCandidate.question_id}`,
      },
    };
  }
...
export default async function EvidenceSeedQuestionPage({
  ...
  if (!detail) {
    const pilotCode = getEvidencePilotCode(seedCandidate.outline_code);
      ...
      <>
        <section className="hero">
          <div className="container">
            <div className="hero-meta">
              ...
              <span className="stamp">{label(seedCandidate.seed_bucket)}</span>
            </div>
            ...
            >
              This candidate is queued for full case-study authoring.
            </h1>
            <p className="body-lg" style={{ marginBottom: 0, maxWidth: "70ch" }}>
              The row is addressable now so review, C3 tagging, answer-flow
              authoring, LeadMe, and Outline Atlas linking can attach to a stable
              route.
            </p>
            <div className="hero-actions" style={{ marginTop: 32 }}>
              ...
              <Link
                href={`${JESUSLOVESYOU_ROUTE_PREFIX}/evidence-pilot-01/${seedCandidate.outline_code}`}
                className="btn btn-lg ghost"
              >
        ...
        <section className="section">
          <div className="container">
            ...
            >
              {[
                ["Recommended code", seedCandidate.outline_code],
                ["Source code", seedCandidate.source_outline_code],
                ["Official key", seedCandidate.key ?? "-"],
                [
                  "Correct percent",
                  seedCandidate.correct_percent
                    ? `${seedCandidate.correct_percent}%`
                    : "n/a",
                ],
                ["Coverage", label(seedCandidate.coverage_group)],

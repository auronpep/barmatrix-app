...
const SUBJECT = "Civil Procedure";
const SUBJECT_SLUG = "civil-procedure";
const DRILL_LIMIT = 6;
...
const PREVIEW_ROWS = [
  ["Trap focus", "Jurisdiction, finality, preclusion, and review posture"],
  ["Forensics rail", "Why the chosen answer looked procedurally available"],
  ["Repair move", "Name the gate before applying the merits rule"],
] as const;
...
async function fetchCivilProcedureQueue(): Promise<SubjectQuestionRef[]> {
...
export default function CivilProcedureDrillPage() {
  ...
  const loadQueue = async () => {
    ...
    try {
      const nextQueue = await fetchCivilProcedureQueue();
      setQueue(nextQueue);

      if (nextQueue.length === 0) {
        setQuestion(null);
        setError("The Civil Procedure drill queue is empty.");
        setPhase("error");
        return;
      }
  ...
  const submit = async () => {
    ...
    try {
      if (setIdRef.current === null) {
        setIdRef.current = `${SUBJECT_SLUG}-inline-${Date.now()}`;
      }
      ...
      <div className="mb-10 grid gap-6 border-b border-zinc-200 pb-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
        <div>
          <p className="font-mono text-xs uppercase tracking-wider text-red-700">
            {BRAND} procedure repair
          </p>
          <h1 className="mt-3 max-w-3xl font-serif text-4xl font-semibold tracking-tight text-zinc-950 sm:text-5xl">
            Civil Procedure drills with forensics beside the question.
          </h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-600">
            Work a focused Civil Procedure queue, submit an answer, and review
            the procedural trap and repair assignment in
            the same surface.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <button
              ...
              className="btn red"
            >
              Start Civil Procedure drill
            </button>
            <Link
              href="/diagnostic"
              className="rounded-md border border-zinc-300 px-5 py-3 text-sm font-medium text-zinc-950 hover:bg-zinc-50"
            >
              Run diagnostic
            </Link>
          </div>
        ...
        <div className="border border-zinc-200 bg-zinc-50 p-5">
          ...
          <p className="mt-2 text-sm text-zinc-600">
            {currentRef
              ...
              : "Start the drill to sync the first Civil Procedure queue."}
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
          <div>
            ...
            {(phase === "loading-bank" || phase === "loading-question") && (
              <StatusPanel title="Loading Civil Procedure drill" />
            )}
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-2xl font-semibold tracking-tight text-zinc-950">
        Start with the Civil Procedure queue.
      </h2>
  ...
  return (
    <div className="border border-zinc-200 bg-zinc-50 p-8">
      ...
      <p className="mt-3 text-zinc-600">
        Syncing the Civil Procedure queue and preparing the first drill question.
      </p>
    </div>
  );
}
...
function ErrorPanel({
  ...
  return (
    <div className="border border-red-200 bg-red-50 p-8">
      <p className="font-medium text-red-900">Civil Procedure drill unavailable.</p>
  ...
  if (phase === "forensics" && attempt && forensics) {
    return (
      <aside className="border border-zinc-300 bg-white p-6 shadow-sm lg:sticky lg:top-6">
        ...
        >
          {isLast ? "Finish drill" : "Next Civil Procedure question"}
        </button>
      </aside>
    );
  }
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-3xl font-semibold tracking-tight text-zinc-950">
        Civil Procedure queue complete.
      </h2>
      <p className="mt-4 max-w-2xl text-zinc-600">
        You worked {total} Civil Procedure question{total === 1 ? "" : "s"} through
        the inline forensics surface. Continue to the Red-Zone Map or restart
        the queue for another pass.
      </p>
      <div className="mt-6 flex flex-wrap gap-3">
        <button
          type="button"

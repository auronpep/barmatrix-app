...
const PREVIEW_ROWS = [
  ["Trap focus", "Scrutiny choice, state action, federal powers, and speech gates"],
  ["Forensics rail", "Why the chosen answer looked constitutionally available"],
  ["Repair move", "Name the threshold gate before applying the merits rule"],
] as const;
...
async function fetchConstitutionalLawQueue(): Promise<SubjectQuestionRef[]> {
...
export default function ConstitutionalLawDrillPage() {
  ...
  const loadQueue = async () => {
    ...
    try {
      const nextQueue = await fetchConstitutionalLawQueue();
      setQueue(nextQueue);

      if (nextQueue.length === 0) {
        setQuestion(null);
        setError("The Constitutional Law drill queue is empty.");
        setPhase("error");
        return;
      }
      ...
      <div className="mb-10 grid gap-6 border-b border-zinc-200 pb-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
        <div>
          <p className="font-mono text-xs uppercase tracking-wider text-red-700">
            {BRAND} constitutional repair
          </p>
          <h1 className="mt-3 max-w-3xl font-serif text-4xl font-semibold tracking-tight text-zinc-950 sm:text-5xl">
            Constitutional Law drills with forensics beside the question.
          </h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-600">
            Work a focused Constitutional Law queue, submit an answer, and
            review the threshold trap and repair assignment
            in the same surface.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <button
              ...
              className="btn red"
            >
              Start Constitutional Law drill
            </button>
            <Link
              href="/diagnostic"
        ...
        <div className="border border-zinc-200 bg-zinc-50 p-5">
          ...
          <p className="mt-2 text-sm text-zinc-600">
            {currentRef
              ...
              : "Start the drill to sync the first Constitutional Law queue."}
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
          <div>
            ...
            {(phase === "loading-bank" || phase === "loading-question") && (
              <StatusPanel title="Loading Constitutional Law drill" />
            )}
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-2xl font-semibold tracking-tight text-zinc-950">
        Start with the Constitutional Law queue.
      </h2>
  ...
  return (
    <div className="border border-zinc-200 bg-zinc-50 p-8">
      ...
      <p className="mt-3 text-zinc-600">
        Syncing the Constitutional Law queue and preparing the first drill
        question.
      </p>
    </div>
  );
  ...
  return (
    <div className="border border-red-200 bg-red-50 p-8">
      <p className="font-medium text-red-900">
        Constitutional Law drill unavailable.
      </p>
  ...
  if (phase === "forensics" && attempt && forensics) {
    return (
      <aside className="border border-zinc-300 bg-white p-6 shadow-sm lg:sticky lg:top-6">
        ...
        >
          {isLast ? "Finish drill" : "Next Constitutional Law question"}
        </button>
      </aside>
    );
  }
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-3xl font-semibold tracking-tight text-zinc-950">
        Constitutional Law queue complete.
      </h2>
      <p className="mt-4 max-w-2xl text-zinc-600">
        You worked {total} Constitutional Law question{total === 1 ? "" : "s"}{" "}
        through the inline forensics surface. Continue to the Red-Zone Map or
        restart the queue for another pass.
      </p>
      <div className="mt-6 flex flex-wrap gap-3">
        <button type="button" onClick={onRestart} className="btn red">
          Restart drill
        </button>
        <Link
          href="/red-zones"

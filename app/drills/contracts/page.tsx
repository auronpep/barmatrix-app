...
const CONTRACTS_PREVIEW = {
  trap: "UCC 2-207 - Form Battle",
  studentSelected: "No contract because the forms differ",
  focusGroupLine: "Focus-group comparison appears after submit",
  forensicTag: "Mirror-image import",
  whyLookedRight:
    "The mirror-image rule is familiar from common-law formation, so a mismatched form feels like no acceptance.",
  whyFails:
    "Goods contracts can form even when the forms do not match. Formation and term inclusion are separate questions.",
  nextDrill: "UCC 2-207 Form Battle Repair",
} as const;
...
async function fetchContractsQueue(): Promise<SubjectQuestionRef[]> {
...
export default function ContractsDrillPage() {
  ...
  const loadQueue = async () => {
    ...
    try {
      const nextQueue = await fetchContractsQueue();
      setQueue(nextQueue);

      if (nextQueue.length === 0) {
        setQuestion(null);
        setError("The Contracts drill queue is empty.");
        setPhase("error");
        return;
      }
  ...
  const submit = async () => {
    ...
    try {
      if (setIdRef.current === null) {
        setIdRef.current = `contracts-inline-${Date.now()}`;
      }
      ...
      <div className="mb-10 grid gap-6 border-b border-zinc-200 pb-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
        <div>
          ...
          <h1 className="mt-3 max-w-3xl font-serif text-4xl font-semibold tracking-tight text-zinc-950 sm:text-5xl">
            Contracts drills with wrong-answer forensics in the same workflow.
          </h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-600">
            Work a Contracts queue, submit an answer, and review the trap and assigned repair drill without leaving the page.
          </p>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
          <div>
            ...
            {(phase === "loading-bank" || phase === "loading-question") && (
              <StatusPanel title="Loading Contracts drill" />
            )}
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-2xl font-semibold tracking-tight text-zinc-950">
        Start with the Contracts queue.
      </h2>
  ...
  return (
    <div className="border border-zinc-200 bg-zinc-50 p-8">
      ...
      <p className="mt-3 text-zinc-600">
        Syncing the Contracts queue and preparing the first drill question.
      </p>
    </div>
  );
}
...
function ErrorPanel({
  ...
  return (
    <div className="border border-red-200 bg-red-50 p-8">
      <p className="font-medium text-red-900">Contracts drill unavailable.</p>
  ...
  return (
    <aside className="border border-zinc-200 bg-zinc-50 p-6 lg:sticky lg:top-6">
      ...
      <dl className="mt-5 space-y-4">
        <PreviewRow label="Student selected" value={CONTRACTS_PREVIEW.studentSelected} />
        <PreviewRow label="Forensic tag" value={CONTRACTS_PREVIEW.forensicTag} />
        <PreviewRow label="Next drill" value={CONTRACTS_PREVIEW.nextDrill} />
      </dl>
      <ForensicsSection title="Why it looked right">
        {CONTRACTS_PREVIEW.whyLookedRight}
      </ForensicsSection>
      <ForensicsSection title="Why it fails">
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-3xl font-semibold tracking-tight text-zinc-950">
        Contracts queue complete.
      </h2>
      <p className="mt-4 max-w-2xl text-zinc-600">
        You worked {total} Contracts question{total === 1 ? "" : "s"} through

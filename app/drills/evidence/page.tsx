...
async function fetchEvidenceQueue(): Promise<SubjectQuestionRef[]> {
...
export default function EvidenceDrillPage() {
  ...
  const loadQueue = async () => {
    ...
    try {
      const nextQueue = await fetchEvidenceQueue();
      setQueue(nextQueue);

      if (nextQueue.length === 0) {
        setQuestion(null);
        setError("The Evidence drill queue is empty.");
        setPhase("error");
        return;
      }
  ...
  const submit = async () => {
    ...
    try {
      if (setIdRef.current === null) {
        setIdRef.current = `evidence-inline-${Date.now()}`;
      }
      ...
      <div className="mb-10 grid gap-6 border-b border-zinc-200 pb-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
        <div>
          ...
          <h1 className="mt-3 max-w-3xl font-serif text-4xl font-semibold tracking-tight text-zinc-950 sm:text-5xl">
            Evidence questions with wrong-answer forensics in the same drill.
          </h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-600">
            Work the Evidence queue, submit an answer, and review the trap and assigned repair drill without leaving the page.
          </p>
        ...
        <div className="border border-zinc-200 bg-zinc-50 p-5">
          ...
          <p className="mt-2 text-sm text-zinc-600">
            {currentRef
              ...
              : "Start the drill to sync the first Evidence queue."}
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
          <div>
            ...
            {(phase === "loading-bank" || phase === "loading-question") && (
              <StatusPanel title="Loading Evidence drill" />
            )}
  ...
  return (
    <div className="border border-zinc-200 bg-zinc-50 p-8">
      ...
      <p className="mt-3 text-zinc-600">
        Syncing the Evidence queue and preparing the first drill question.
      </p>
    </div>
  );
}
...
function ErrorPanel({
  ...
  return (
    <div className="border border-red-200 bg-red-50 p-8">
      <p className="font-medium text-red-900">Evidence drill unavailable.</p>
      {message && <p className="mt-2 font-mono text-xs text-red-800">{message}</p>}
      <button
        type="button"
        onClick={onRetry}
        className="mt-6 rounded-md border border-red-300 px-5 py-2.5 text-sm font-medium text-red-950 hover:bg-red-100"
      >
        Retry
      </button>
    </div>
  ...
  return (
    <aside className="border border-zinc-200 bg-zinc-50 p-6 lg:sticky lg:top-6">
      ...
      <dl className="mt-5 space-y-4">
        <PreviewRow label="Student selected" value={PROOF_CARD.studentSelected} />
        <PreviewRow label="Forensic tag" value={PROOF_CARD.forensicTag} />
        <PreviewRow label="Next drill" value={PROOF_CARD.nextDrill} />
      </dl>
      <p className="mt-5 text-sm leading-6 text-zinc-600">
        Submit an answer to replace this proof card with the live attempt
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <p className="mt-4 max-w-2xl text-zinc-600">
        You worked {total} Evidence question{total === 1 ? "" : "s"} through the

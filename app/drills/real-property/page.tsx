...
const PREVIEW_ROWS = [
  ["Trap focus", "Recording, ownership, future interests, and land-use traps"],
  ["Forensics rail", "Why the chosen answer looked property-law available"],
  ["Repair move", "Name the property interest before choosing the rule"],
] as const;
...
async function fetchRealPropertyQueue(): Promise<SubjectQuestionRef[]> {
...
export default function RealPropertyDrillPage() {
  ...
  const loadQueue = async () => {
    ...
    try {
      const nextQueue = await fetchRealPropertyQueue();
      setQueue(nextQueue);

      if (nextQueue.length === 0) {
        setQuestion(null);
        setError("The Real Property drill queue is empty.");
        setPhase("error");
        return;
      }
      ...
      <div className="mb-10 grid gap-6 border-b border-zinc-200 pb-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
        <div>
          <p className="font-mono text-xs uppercase tracking-wider text-red-700">
            {BRAND} property repair
          </p>
          <h1 className="mt-3 max-w-3xl font-serif text-4xl font-semibold tracking-tight text-zinc-950 sm:text-5xl">
            Real Property drills with forensics beside the question.
          </h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-600">
            Work a focused Real Property queue, submit an answer, and
            review the property trap and repair assignment
            in the same surface.
          </p>
        ...
        <div className="border border-zinc-200 bg-zinc-50 p-5">
          ...
          <p className="mt-2 text-sm text-zinc-600">
            {currentRef
              ...
              : "Start the drill to sync the first Real Property queue."}
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
          <div>
            ...
            {(phase === "loading-bank" || phase === "loading-question") && (
              <StatusPanel title="Loading Real Property drill" />
            )}
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-2xl font-semibold tracking-tight text-zinc-950">
        Start with the Real Property queue.
      </h2>
  ...
  return (
    <div className="border border-zinc-200 bg-zinc-50 p-8">
      ...
      <p className="mt-3 text-zinc-600">
        Syncing the Real Property queue and preparing the first drill
        question.
      </p>
    </div>
  );
  ...
  return (
    <div className="border border-red-200 bg-red-50 p-8">
      <p className="font-medium text-red-900">
        Real Property drill unavailable.
      </p>
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-3xl font-semibold tracking-tight text-zinc-950">
        Real Property queue complete.
      </h2>
      <p className="mt-4 max-w-2xl text-zinc-600">
        You worked {total} Real Property question{total === 1 ? "" : "s"}{" "}

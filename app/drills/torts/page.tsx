...
const TORTS_PREVIEW = {
  trap: "Negligence - Duty and Scope",
  studentSelected: "Liability follows every foreseeable harm",
  focusGroupLine: "Focus-group comparison appears after submit",
  forensicTag: "Foreseeability overreach",
  whyLookedRight:
    "Foreseeability is a familiar negligence word, so it can feel like it answers duty, breach, causation, and scope at once.",
  whyFails:
    "Torts questions often separate the existence of a duty from the scope of liability and the specific risk that made the conduct negligent.",
  nextDrill: "Duty, Scope, and Risk Field Repair",
} as const;
...
async function fetchTortsQueue(): Promise<SubjectQuestionRef[]> {
...
export default function TortsDrillPage() {
  ...
  const loadQueue = async () => {
    ...
    try {
      const nextQueue = await fetchTortsQueue();
      setQueue(nextQueue);

      if (nextQueue.length === 0) {
        setQuestion(null);
        setError("The Torts drill queue is empty.");
        setPhase("error");
        return;
      }
  ...
  const submit = async () => {
    ...
    try {
      if (setIdRef.current === null) {
        setIdRef.current = `torts-inline-${Date.now()}`;
      }
      ...
      <div className="mb-10 grid gap-6 border-b border-zinc-200 pb-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
        <div>
          ...
          <h1 className="mt-3 max-w-3xl font-serif text-4xl font-semibold tracking-tight text-zinc-950 sm:text-5xl">
            Torts drills with wrong-answer forensics in the same workflow.
          </h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-zinc-600">
            Work a Torts queue, submit an answer, and review the trap and assigned repair drill without leaving the page.
          </p>
        ...
        <div className="border border-zinc-200 bg-zinc-50 p-5">
          ...
          <p className="mt-2 text-sm text-zinc-600">
            {currentRef
              ...
              : "Start the drill to sync the first Torts queue."}
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
          <div>
            ...
            {(phase === "loading-bank" || phase === "loading-question") && (
              <StatusPanel title="Loading Torts drill" />
            )}
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-2xl font-semibold tracking-tight text-zinc-950">
        Start with the Torts queue.
      </h2>
      <p className="mt-3 max-w-2xl text-zinc-600">
        The first click syncs live Torts questions. After each answer, the right
  ...
  return (
    <div className="border border-zinc-200 bg-zinc-50 p-8">
      ...
      <p className="mt-3 text-zinc-600">
        Syncing the Torts queue and preparing the first drill question.
      </p>
    </div>
  );
}
...
function ErrorPanel({
  ...
  return (
    <div className="border border-red-200 bg-red-50 p-8">
      <p className="font-medium text-red-900">Torts drill unavailable.</p>
  ...
  return (
    <aside className="border border-zinc-200 bg-zinc-50 p-6 lg:sticky lg:top-6">
      ...
      <dl className="mt-5 space-y-4">
        <PreviewRow label="Student selected" value={TORTS_PREVIEW.studentSelected} />
        <PreviewRow label="Forensic tag" value={TORTS_PREVIEW.forensicTag} />
        <PreviewRow label="Next drill" value={TORTS_PREVIEW.nextDrill} />
      </dl>
      <ForensicsSection title="Why it looked right">
        {TORTS_PREVIEW.whyLookedRight}
      </ForensicsSection>
      <ForensicsSection title="Why it fails">
  ...
  return (
    <div className="border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-3 font-serif text-3xl font-semibold tracking-tight text-zinc-950">
        Torts queue complete.
      </h2>
      <p className="mt-4 max-w-2xl text-zinc-600">
        You worked {total} Torts question{total === 1 ? "" : "s"} through the
        inline forensics surface. Continue to the Red-Zone Map or restart the

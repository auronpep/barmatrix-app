...
const SUBJECT_SLUG = "contracts";
const PAGE_SIZE = 12;

const SUBJECT_FOCUS = {
  heading: "Contracts Focus",
  label: "Formation, performance, breach, and remedies",
  body:
    "Contracts practice starts from the returned topic and tension metadata, then routes into the same forensic question runner used by the diagnostic flow.",
} as const;
...
export default function ContractsSubjectPage() {
  ...
  const startPractice = () => {
    ...
    if (questionIds.length === 0) {
      setPracticeError("The Contracts question queue is empty.");
      return;
    }
    ...
    try {
      ...
      setPracticeError("This browser could not cache the Contracts practice queue.");
      return;
    }
      ...
      <section className="hero">
        <div className="container">
          ...
          <p className="body-lg" style={{ marginBottom: 0, maxWidth: 760 }}>
            Contracts practice starts from the live subject bank, then moves into
            the same question and forensics runner used by the diagnostic flow.
          </p>
          <div className="hero-actions" style={{ marginTop: 32 }}>
            <button
              ...
              disabled={phase !== "ready" || questionIds.length === 0}
            >
              Start Contracts practice <span className="arrow">-&gt;</span>
            </button>
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="two-col" style={{ alignItems: "start" }}>
            <div>
              {phase === "loading" && <StatusPanel title="Loading Contracts bank" />}
...
function StatusPanel({ title }: { title: string }) {
  return (
    <div className="info-panel">
      ...
      <p style={{ margin: 0, color: "var(--ink-soft)" }}>
        Syncing the first Contracts page from the subject endpoint.
      </p>
    </div>
  );
}
  ...
  if (questions.length === 0) {
    return (
      <div className="info-panel alert">
        <div className="eyebrow-strong" style={{ marginBottom: 12 }}>
          No Contracts questions returned
        </div>
        <p style={{ margin: 0 }}>

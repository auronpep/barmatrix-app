...
function normalizeBank(payload: unknown): SubjectBank {
  ...
  if (isRecord(payload)) {
    ...
    limit = asNumber(payload.limit) ?? PER_SUBJECT_LIMIT;
  }
...
async function fetchSubjectBank(subject: string): Promise<SubjectBank> {
...
function combineBanks(banks: SubjectBank[]): SubjectBank {
  return {
    questions: banks.flatMap((bank) => bank.questions),
    total: banks.reduce((sum, bank) => sum + bank.total, 0),
    page: 1,
    limit: banks.reduce((sum, bank) => sum + bank.limit, 0),
    loadedAt: new Date().toISOString(),
  };
}
...
export default function CriminalLawSubjectPage() {
  ...
  const syncBank = async () => {
    ...
    try {
      const banks = await Promise.all(
        SUBJECTS.map((subject) => fetchSubjectBank(subject)),
      );
      setBank(combineBanks(banks));
      setPhase("ready");
    } catch (nextError) {
  ...
  const startPractice = () => {
    ...
    if (questionIds.length === 0) {
      setPracticeError("Sync the Criminal Law and Procedure banks before starting practice.");
      return;
    }
    ...
    try {
      window.sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          expected_total: PER_SUBJECT_LIMIT * SUBJECTS.length,
          bank_loaded: true,
          subject: "Criminal Law and Procedure",
        }),
      );
    } catch {
      setPracticeError("This browser could not cache the criminal-law queue.");
      return;
    }
      ...
      <section className="hero">
        <div className="container">
          ...
          <div className="hero-actions" style={{ marginTop: 32 }}>
            ...
            >
              {phase === "loading" ? "Syncing banks" : "Sync criminal banks"}
        ...
        <div className="container">
          ...
          >
            <div>
              ...
              {phase === "loading" && <StatusPanel title="Loading criminal-law banks" />}
            ...
            >
              ...
              <div className="forensics-body">
                ...
                >
                  Criminal questions are routed across elements, mental states,
                  defenses, search and seizure, interrogation, counsel, and trial
                  rights so the practice flow can isolate doctrine-switching
                  traps.
                </p>

                <SummaryBlock title="Subjects" items={subjects} />
